...
{ dispatch({ type: 'START_LOADING' });
  ...
  if (result) {
    console.log('actions/user.js-register-res=',result)
    dispatch({ type: 'UPDATE_USER', payload: result });
    dispatch({ type: 'CLOSE_LOGIN' });
    dispatch({ type: 'UPDATE_ALERT',
      payload: {open: true,severity: 'success',message: 'Your account has been created successfully',},
            });
  }
...
export const login = async (user, dispatch) => {
  ...
  if (result) {
    console.log('actions/user.js-login-res=',result)
    dispatch({ type: 'UPDATE_USER', payload: result });
    dispatch({ type: 'CLOSE_LOGIN' });
  }
  dispatch({ type: 'END_LOADING' });
...
export const updateProfile = async (currentUser, updatedFields, dispatch) => {
  ...
  try {
    ...
    const result = await fetchData({ url: url + '/updateProfile', method: 'POST',body,token: currentUser.token,},dispatch);
    if (result) {
  ...
  } catch (error) { dispatch({type: 'UPDATE_ALERT',payload: {open: true,severity: 'error', message: error.message,},});
    console.log(error);
  }
  dispatch({ type: 'END_LOADING' });
};