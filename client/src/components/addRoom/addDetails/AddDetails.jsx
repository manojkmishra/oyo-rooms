...
const AddDetails = () => {
  ...
  { const costType = Number(e.target.value);
    ...
    if (costType === 0) { dispatch({ type: 'UPDATE_DETAILS', payload: { price: 0 } });  }
    else { dispatch({ type: 'UPDATE_DETAILS', payload: { price: 15 } });  }
  };
  ...
  return (
    <Stack  sx={{ alignItems: 'center', '& .MuiTextField-root': { width: '100%', maxWidth: 500, m: 1 }, }}>
        ...
        <RadioGroup name="costType" value={costType} row onChange={handleCostTypeChange}>
          ...
          {Boolean(costType) && (
            <TextField   sx={{ width: '7ch !important' }}  variant="standard"
              ...
              inputProps={{ type: 'number', min: 1, max: 50 }} value={price}
      ...
      <InfoField mainProps={{name: 'description', label: 'Description', value: description,}}
        minLength={10} optionalProps={{ multiline: true, rows: 4 }}/>
    </Stack>
  );
};
